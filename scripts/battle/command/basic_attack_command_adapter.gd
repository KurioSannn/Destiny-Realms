extends Node
class_name BasicAttackCommandAdapter

signal basic_ready(command)
signal target_changed(command, targets)
signal basic_cancelled(command)
signal basic_committed(command)
signal basic_failed(command, reason)
...
func configure(
	...
	if flow == null:
		...
		flow.name = "BasicAttackCommandFlow"
		add_child(flow)
...
func begin_basic(source_turn: int = 0) -> bool:
	if flow == null:
		return false
...
func fail_basic(
	...
	basic_failed.emit(command, reason)


func has_pending_basic() -> bool:
	return (
		flow != null
		and flow.has_pending_command()
		and flow.get_pending_command().command_type == PendingCommand.CommandType.BASIC_ATTACK
	)
