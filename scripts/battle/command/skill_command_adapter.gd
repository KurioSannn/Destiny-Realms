extends Node
class_name SkillCommandAdapter

signal skill_ready(command)
signal target_changed(command, targets)
signal skill_cancelled(command)
signal skill_committed(command)
signal skill_failed(command, reason)
...
func fail_skill(
	...
	skill_failed.emit(command, reason)
...
func has_pending_skill() -> bool:
	return (
		...
		and flow.get_pending_command().command_type == PendingCommand.CommandType.SKILL
	)
